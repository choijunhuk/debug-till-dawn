...
export class WeaponSystem {
    ...
    has(weaponId) { return this.owned.some((w) => w.def.id === weaponId); }
    get(weaponId) { return this.owned.find((w) => w.def.id === weaponId); }
    levelUp(weaponId) { const w = this.get(weaponId); if (w && w.level < w.def.maxLevel)
        w.level++; }
    isMaxed(weaponId) { const w = this.get(weaponId); return !!w && w.level >= w.def.maxLevel; }
    // 진화: 재료 무기 제거 + 결과 무기 추가
    evolve(materialId, resultId) {
        const w = this.get(materialId);
        if (w)
            w.orbList.forEach((o) => o.kill());
        if (w?.aura)
            w.aura.destroy();
    ...
    update(ctx, delta) {
        for (const w of this.owned) {
            switch (w.def.behavior) {
                ...
                case 'pierce':
                case 'homing':
                    this.fireRanged(w, ctx);
                    break;
                case 'bounce':
                    this.fireRanged(w, ctx);
                    break;
                case 'orbit':
                    this.updateOrbit(w, ctx, delta);
                    break;
                case 'whip':
                    this.fireWhip(w, ctx);
                    break;
                case 'aura':
                    this.updateAura(w, ctx);
                    break;
                case 'summon':
                    this.fireSummon(w, ctx);
                    break;
                case 'nuke':
                    this.fireNuke(w, ctx);
                    break;
    ...
    fireRanged(w, ctx) {
        ...
        if (ctx.now - w.lastFire < s.cooldown)
            return;
    ...
    updateOrbit(w, ctx, delta) {
        const s = this.statsFor(w, ctx);
        // 오브 개수 맞추기
        while (w.orbList.length < s.count) {
            const o = ctx.orbs.get();
            if (!o)
                break;
    ...
    updateAura(w, ctx) {
        ...
        for (const e of enemies) {
            if (w.def.slow)
                e.slowUntil = ctx.now + 400; // Docker 속박
