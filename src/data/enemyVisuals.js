...
export function shapeSize(shape) { return BOSS_SHAPES.includes(shape) ? 96 : ESHAPE_SIZE; }
export function drawEnemyShape(g, shape) {
    switch (shape) {
...
function drawMergeConflict(g) {
    ...
    g.moveTo(24, 11);
    for (let y = 11, k = 0; y <= 37; y += 6, k++)
        g.lineTo(24 + (k % 2 ? 4 : -4), y);
    g.strokePath();
}
// Infinite Loop: 회전 화살표 고리(↻). 실제 회전은 step 에서.
function drawInfLoop(g) {
    const cyan = 0x4ec9b0;
...
// Deadlock: 맞물린 톱니 두 개
function drawDeadlock(g) {
    const o = 0xce9178;
    for (const [gx] of [[18], [30]]) {
        g.lineStyle(2.5, o, 1).strokeCircle(gx, C, 7);
        for (let a = 0; a < 8; a++) {
            const ang = a / 8 * Math.PI * 2;
            g.lineBetween(gx + Math.cos(ang) * 7, C + Math.sin(ang) * 7, gx + Math.cos(ang) * 10, C + Math.sin(ang) * 10);
        }
...
const BC = 48;
// O(n²): 노드 격자 + 연결선
function drawBossOnSquared(g) {
    const purple = 0xc586c0;
    const pts = [];
    for (let i = 0; i < 3; i++)
        for (let j = 0; j < 3; j++)
            pts.push({ x: 24 + i * 24, y: 24 + j * 24 });
...
function drawBossSegfault(g) {
    ...
    for (let i = 0; i < 4; i++)
        for (let j = 0; j < 4; j++) {
            if ((i * 3 + j) % 4 === 0)
                continue;
            g.fillRect(14 + i * 18, 14 + j * 18, 14, 14);
        }
    g.lineStyle(3, 0x0d1117, 1).lineBetween(18, 48, 78, 38).lineBetween(48, 14, 54, 82);
}
// Production Bug: 거대 버그/스컬 + 다리
function drawBossProdBug(g) {
    const pink = 0xff0066;
    g.lineStyle(4, pink, 1);
    for (let a = 0; a < 8; a++) {
        const ang = a / 8 * Math.PI * 2;
        g.lineBetween(BC + Math.cos(ang) * 28, BC + 4 + Math.sin(ang) * 28, BC + Math.cos(ang) * 40, BC + 4 + Math.sin(ang) * 40);
    }
