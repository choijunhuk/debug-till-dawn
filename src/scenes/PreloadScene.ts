...
export class PreloadScene extends Phaser.Scene {
  ...
  private loadLogos(): Promise<void> {
    const tasks = Object.entries(brandAssets).map(([key, brand]) => new Promise<void>((res) => {
      ...
      img.onload = () => {
        ...
        if (this.textures.exists(`logo_${key}`)) this.textures.remove(`logo_${key}`);
  ...
  private circle(key: string, r: number, color: number) {
    const g = this.add.graphics();
