...
export interface UpgradeOption {
  ...
  icon?: string;  // 무기 로고 키(brandAssets)
}
...
export class LevelUpCards {
  ...
  show(options: UpgradeOption[], onPick: () => void) {
    ...
    options.forEach((opt, i) => {
      ...
      if (opt.gold) this.scene.tweens.add({ targets: bg, alpha: 0.7, duration: 400, yoyo: true, repeat: -1 });
    ...
    this.keyHandler = (e: KeyboardEvent) => {
      ...
      if (idx >= 0 && idx < options.length) this.pick(options[idx], onPick);
    };
